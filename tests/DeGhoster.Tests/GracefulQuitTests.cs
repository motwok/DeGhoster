...
// Exercises DeGhoster's *graceful* shutdown (tray-style Exit), which the kill-based
// neutralization test never triggers: it runs GhostEngine/HookInjector teardown
// (removeAll -> DgRemoveHook, helper cleanup) and the DLL's auto-uncloak. Covers
...
public class GracefulQuitTests
{
    ...
    public void Graceful_exit_tears_down_cleanly(string bits)
    {
        ...
        try
        {
            ...
            // Graceful shutdown: post the Exit command to DeGhoster's main window.
