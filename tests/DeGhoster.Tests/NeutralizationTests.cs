...
// End-to-end: GhostSim creates a real ghost window (class Chrome_WidgetWin_1,
// layered, alpha 0 + LWA_ALPHA, visible, on-screen); DeGhoster must detect and
// neutralize it (DWMWA_CLOAKED set). Covers both injection paths: x64 direct
// (Hook64) and x86 via Helper32 (Hook32). Requires an interactive desktop + DWM.
...
public class NeutralizationTests
{
    ...
    public void DeGhoster_neutralizes_ghost_window(string bits)
    {
        ...
        try
        {
            ...
            Assert.Equal(0, Cloaked(h));  // precondition: a real, un-cloaked ghost
