...
// Drives the GUI through window messages (no pixel clicks / no modal menus) to
// exercise MainWindow command handlers, the About window (InfoWindow paint), the
// RTL layout path, and the GhostEngine window-destroy path.
...
public class UiCoverageTests
{
    ...
    public void Info_window_power_toggle_and_quit()
    {
        ...
        try
        {
            // Open the About window (owner-draw paint runs), then close it.
            ...
            // Toggle the global switch off and on (setGlobalEnabled + refreshAll).
            PostMessage(host, WM_COMMAND, (IntPtr)IDC_POWER, IntPtr.Zero);
            Thread.Sleep(300);
            PostMessage(host, WM_COMMAND, (IntPtr)IDC_POWER, IntPtr.Zero);
            Thread.Sleep(300);

            // Hover the toolbar buttons (HoverButton hot/leave + hot owner-draw).
    ...
    public void Tray_menu_toggles_a_window()
    {
        ...
        try
        {
            ...
            // Selecting the per-window entry toggles it off -> it un-cloaks.
    ...
    public void Closing_a_ghost_drops_it_from_tracking()
    {
        ...
        try
        {
            // Close the ghost window -> GhostEngine gets EVENT_OBJECT_DESTROY.
