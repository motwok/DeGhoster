...
#
# OPTIONAL, human-guided coverage run. It covers the lines the automated tests
# cannot reach robustly (tray menu, per-window eye click, DPI change, 32-bit
# helper teardown) by having YOU perform those actions on screen while the app
# runs under OpenCppCoverage. The result is merged with the automated coverage
# (coverage\auto.cov from tests\coverage.ps1) into coverage\merged-html.
#
# Usage:
#   .\tests\coverage.ps1          # run the automated run first (writes auto.cov)
#   .\tests\coverage-manual.ps1   # then this, and follow the on-screen steps
#
# Requires OpenCppCoverage (choco install opencppcoverage) and a real desktop.
...
# Merge with the automated coverage if present.
