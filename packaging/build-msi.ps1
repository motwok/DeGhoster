...
#
# Builds the DeGhoster MSI (WiX v4/v6/v7 `wix` CLI) from the current build/
# output. Produces  dist/DeGhoster-<FullSemVer>-win-x64.msi
#
#   * Dual scope: per-user OR per-machine (chosen in the setup).
#   * Per-language feature selection (all languages on by default).
#   * Autostart always per-user (HKCU Run); per-machine installs use Active
#     Setup so every user is registered at logon.
#
# Assumes the binaries are already built (run build.ps1 first). Requires the
# WiX CLI and the WixToolset.UI.wixext extension:
